...
const RAW_ORACLE_URL = (import.meta.env && import.meta.env.VITE_ORACLE_URL) || "";
// Be forgiving: add https:// if it was omitted, strip any trailing slash, and
// ignore obvious placeholders so the app never builds a broken relative path.
const ORACLE_URL = (() => {
  let u = RAW_ORACLE_URL.trim();
  if (!u || u === "https://x" || u === "x") return "";
...
// QA: send the article URL + judgment criteria; Worker fetches, runs deterministic
// checks, and scores judgment in one call. Returns the full result object:
// { title, deterministic, detail, criteria }.
export async function assessQA({ url, title, category, criteria }) {
  return callWorker("/qa", { url, title, category, judgmentCriteria: criteria });
}
